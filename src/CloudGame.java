...
public class CloudGame {
    ...
    public static int jumpingOnClouds(List<Integer> c) throws Exception {
        int jumps = 0;
        for (int i = 0; i < c.size() -1; i++) {
            if (i+2 < c.size() && c.get(i+2) == 0) {
                ...
                throw new Exception("No Path to jump");
            }
        }
        return jumps;
    }
    ...
    public static int cloud_pilot_jumpingOnClouds(List<Integer> c) {
        int jumps = 0;
        int i = 0;
        while (i < c.size() - 1) {
            if (i + 2 < c.size() && c.get(i + 2) == 0) {
